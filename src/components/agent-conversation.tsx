...
interface Props { run: Run; events: ActivityEvent[]; pendingRequests?: HumanRequest[]; busy?: boolean; onSendContext(content: string): void; }
...
function HumanNeeded({ requests }: { requests: HumanRequest[] }) { const count = requests.length; const heading = count === 1 ? "Your input is needed" : `${count} pending questions`; return <article className="grid grid-cols-[30px_minmax(0,1fr)] gap-2.5"><span className="grid size-[29px] place-items-center rounded-lg bg-amber-200 text-amber-950"><ShieldAlert size={15}/></span><div className="min-w-0 rounded-r-xl rounded-bl-xl border border-amber-300/50 bg-amber-950/40 p-3"><div className="flex items-baseline gap-2"><b className="text-xs text-amber-100">{heading}</b><span className="font-mono text-[10px] text-amber-200/70">agent paused safely</span></div>{count === 1 && <p className="mt-2 text-sm leading-relaxed text-amber-50">{requests[0].question}</p>}{count > 1 && <p className="mt-2 text-sm leading-relaxed text-amber-50">Answer each checkpoint to keep every paused step moving.</p>}<small className="mt-2 block text-xs text-amber-200">Use the human checkpoint{count > 1 ? "s" : ""} beside the browser.</small></div></article>; }
