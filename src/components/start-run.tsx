...
const schema = z.object({ url: z.url("Enter a complete job URL."), task: z.string().max(10_000).optional(), promptVariant: z.string().optional() });
type Form = z.infer<typeof schema>;

export function StartRun({ onSubmit }: { onSubmit(url: string, task: string, promptVariant: string): void }) {
  const form = useForm<Form>({ resolver: zodResolver(schema), defaultValues: { url: "", task: "", promptVariant: "orchestrator.md" } });
  ...
  return <main className="mx-auto grid min-h-[calc(100vh-118px)] max-w-6xl items-center gap-10 px-5 py-12 lg:grid-cols-[1.1fr_.9fr]">
    ...
    <form className="rounded-2xl border border-stone-200 bg-white p-6 shadow-xl shadow-stone-300/30 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-black/20" onSubmit={form.handleSubmit(({ url, task, promptVariant }) => onSubmit(url, task ?? "", promptVariant ?? "orchestrator.md"))}>
      ...
      <label className="mt-4 block text-xs text-stone-700 dark:text-zinc-200">Prompt variant <em className="not-italic text-stone-400 dark:text-zinc-500">experimental</em><select className="mt-2 w-full rounded-lg border border-stone-300 bg-white px-3 py-3 text-stone-950 outline-none focus:border-violet-500 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100" {...form.register("promptVariant")}>{(variants.data ?? []).map((v) => (<option key={v.name} value={v.name}>{v.name}{v.is_default ? " (default)" : ""}</option>))}</select></label>
