...
export function NoVncCanvas({ websocketUrl, viewOnly }: Props) {
  ...
  return <div className="relative h-full min-h-0 w-full overflow-hidden"><div ref={host} className="h-full min-h-0 w-full overflow-hidden [&>*]:max-h-full [&>*]:max-w-full [&_canvas]:object-contain" aria-label="Live browser workspace"/><div className="pointer-events-none absolute top-2 left-2 rounded bg-black/70 px-2 py-1 text-[11px] font-medium uppercase tracking-[0.05em] text-white">{status} · {viewOnly ? "watch" : "control"}</div><div className="absolute top-2 right-2 flex gap-1"><button className="grid size-7 place-items-center rounded bg-black/70 text-white hover:bg-black" onClick={() => setGeneration((value) => value + 1)} title="Reconnect browser view"><RefreshCw size={13}/></button><button className="grid size-7 place-items-center rounded bg-black/70 text-white hover:bg-black" onClick={() => void host.current?.requestFullscreen()} title="Fullscreen browser"><Maximize2 size={13}/></button></div></div>;
}
