...
export function HumanPanel({ run, request, onAnswer, onDecision }: Props) {
  ...
  if (!request) return <section className="rounded-xl border border-slate-700 bg-slate-900 p-5 text-center"><Hand className="mx-auto text-emerald-300" size={24}/><h2 className="mt-2 text-[15px] font-medium text-slate-100">Human checkpoint clear</h2><p className="mx-auto mt-2 max-w-sm text-[11px] leading-relaxed text-slate-400">The agent will ask for input only when browser evidence or candidate memory is insufficient.</p><span className="mt-2 block font-mono text-[8px] text-emerald-300">Approval stays locked until a fresh review passes.</span></section>;
